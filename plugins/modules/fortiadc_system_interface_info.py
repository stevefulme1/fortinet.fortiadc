...
DOCUMENTATION = """
---
module: fortiadc_system_interface_info
short_description: Get network interface info from Fortinet FortiADC.
description:
    - This module retrieves interfaces information from FortiADC.
version_added: "1.0.0"
author:
...
- name: Get all interfaces
  fortinet.fortiadc.fortiadc_system_interface_info:
    vdom: root
  register: result
...
RETURN = """
interfaces:
  description: List of interfaces objects.
  returned: always
  type: list
  elements: dict
"""
    ...
    if module._socket_path:
        ...
        try:
            ...
            if module.params.get("name"):
                ...
                resp = fos.get("system", "interface", vdom=vdom)
            module.exit_json(
                changed=False,
                interfaces=resp.get("results", [resp]),
            )
        except Exception as e:
            module.fail_json(
                msg="Failed to get interfaces: %s" % str(e),
            )
    else:
