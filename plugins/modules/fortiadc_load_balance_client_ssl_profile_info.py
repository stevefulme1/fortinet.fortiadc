...
DOCUMENTATION = """
---
module: fortiadc_load_balance_client_ssl_profile_info
short_description: Get client SSL profile info from Fortinet FortiADC.
description:
    - This module retrieves profiles information from FortiADC.
version_added: "1.0.0"
author:
...
- name: Get all profiles
  fortinet.fortiadc.fortiadc_load_balance_client_ssl_profile_info:
    vdom: root
  register: result
...
RETURN = """
profiles:
  description: List of profiles objects.
  returned: always
  type: list
  elements: dict
"""
    ...
    if module._socket_path:
        ...
        try:
            ...
            if module.params.get("name"):
                resp = fos.get(
                    "load-balance", "client-ssl-profile",
                    vdom=vdom, mkey=module.params["name"],
                )
            else:
                resp = fos.get("load-balance", "client-ssl-profile", vdom=vdom)
            module.exit_json(
                changed=False,
                profiles=resp.get("results", [resp]),
            )
        except Exception as e:
            module.fail_json(
                msg="Failed to get profiles: %s" % str(e),
            )
    else:
