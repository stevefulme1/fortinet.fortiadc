...
DOCUMENTATION = """
---
module: fortiadc_load_balance_content_routing_info
short_description: Get content routing info from Fortinet FortiADC.
description:
    - This module retrieves content routings information from FortiADC.
version_added: "1.0.0"
author:
...
EXAMPLES = """
- name: Get all content routings
  fortinet.fortiadc.fortiadc_load_balance_content_routing_info:
    vdom: root
  register: result

- name: Get a specific content routing
  fortinet.fortiadc.fortiadc_load_balance_content_routing_info:
    vdom: root
    name: "example_name"
  register: result
...
RETURN = """
content_routings:
  description: List of content routings objects.
  returned: always
  type: list
  elements: dict
"""
...
def main():
    ...
    if module._socket_path:
        ...
        try:
            ...
            module.exit_json(
                changed=False,
                content_routings=resp.get("results", [resp]),
            )
        except Exception as e:
            module.fail_json(
                msg="Failed to get content routings: %s" % str(e),
            )
    else:
