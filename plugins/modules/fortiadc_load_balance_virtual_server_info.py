...
DOCUMENTATION = """
---
module: fortiadc_load_balance_virtual_server_info
short_description: Get virtual server info from Fortinet FortiADC.
description:
    - This module retrieves virtual servers information from FortiADC.
version_added: "1.0.0"
author:
...
EXAMPLES = """
- name: Get all virtual servers
  fortinet.fortiadc.fortiadc_load_balance_virtual_server_info:
    vdom: root
  register: result
...
RETURN = """
virtual_servers:
  description: List of virtual servers objects.
  returned: always
  type: list
  elements: dict
"""
    ...
    if module._socket_path:
        ...
        try:
            ...
            if module.params.get("name"):
                ...
                resp = fos.get("load-balance", "virtual-server", vdom=vdom)
            module.exit_json(
                changed=False,
                virtual_servers=resp.get("results", [resp]),
            )
        except Exception as e:
            module.fail_json(
                msg="Failed to get virtual servers: %s" % str(e),
            )
    else:
