...
DOCUMENTATION = """
---
module: fortiadc_load_balance_pool_info
short_description: Get real server pool info from Fortinet FortiADC.
description:
    - This module retrieves pools information from FortiADC.
version_added: "1.0.0"
author:
...
EXAMPLES = """
- name: Get all pools
  fortinet.fortiadc.fortiadc_load_balance_pool_info:
    vdom: root
  register: result
...
RETURN = """
pools:
  description: List of pools objects.
  returned: always
  type: list
  elements: dict
"""
    ...
    if module._socket_path:
        ...
        try:
            ...
            if module.params.get("name"):
                ...
                resp = fos.get("load-balance", "pool", vdom=vdom)
            module.exit_json(
                changed=False,
                pools=resp.get("results", [resp]),
            )
        except Exception as e:
            module.fail_json(
                msg="Failed to get pools: %s" % str(e),
            )
    else:
